...
internals.prepare = async function (response) {
    ...
    try {
        ...
        //save stat so we can use it later
        response.stat = stat;
...
internals.createStream = function (response) {
    ...
    // if options didn't have end set, set it to end of file
    if (options.end === undefined) {
        options.end = options.start + response.stat.size ? response.stat.size - 1 : 0;
    }
